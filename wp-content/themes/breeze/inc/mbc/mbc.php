...
class AT_Meta_Box {
  ...
  public function load_scripts_styles() {
    ...
    if (in_array($typenow,$this->_meta_box['pages']) && $this->is_edit_page()){
      ...
      wp_enqueue_script( 'custom-jquery', $plugin_path . '/js/custom.js', array( 'jquery' ), null, true );
...
public function add($postType) {
  ...
  if(in_array($postType, $this->_meta_box['pages'])){
    if(isset($this->_meta_box['template'])){
       if($this->_meta_box['template'] == $template_file){
  ...
  public function show() {
    ...
    foreach ( $this->_fields as $field ) {
      ...
      if (isset($field['group']) && $field['group'] == 'start'){
        $this->inGroup = true;
        echo '<div><div class="form-table"><div>';
      }
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = get_post_meta($post->ID,$field['id'],true);
      ...
      if (count($meta) > 0 && is_array($meta) ){
         foreach ($meta as $me){
           ...
           if ( in_array( $field['fields'][0]['type'], array('image','file') ) )
            $mmm = $c +1 ;
           echo '<div class="at-repater-block">'.$mmm.'<br/><div class="repeater-table" style="display: none;">';
           if ($field['inline']){
      ...
      call_user_func ( array( $this, 'show_field_' . $f['type'] ), $f, $m,true);
        ...
        echo '</div><div class="re-controls">';
        if ($field['sortable'])
          echo '<span class="re-control re-sort"><img src="'.$plugin_path.'/images/move.png" alt="sort" title="sort" class="at_re_sort_handle" /></span>';

        echo'
        <span class="re-control at-re-toggle"><img src="'.$plugin_path.'/images/edit.png" alt="Edit" title="Edit"/></span> 
        <span class="re-control re-del"><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove','breeze').'" title="'.__('Remove','breeze').'" id="remove-'.$field['id'].'"></span>
        <span class="re-control-clear"></span></div></div>';
        $c = $c + 1;
        }
      }

    echo '<a href="javascript:void(0);" id="add-'.$field['id'].'" class="button button-primary button-large">Add '.@$field['btn'].'</a>';
    ...
    echo '</div><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove','breeze').'" title="'.__('Remove','breeze').'" id="remove-'.$field['id'].'" class="st-remove"></div>';
  ...
  public function show_field_begin( $field, $meta) {
    ...
    if($field['type'] == 'posts'){
      $ptype = $field['options']['post_type'];
      echo ' &nbsp; &nbsp;<a href="'.(admin_url('post-new.php?post_type=').$ptype).'">Create '.( isset($field['new']) ? $field['new'] : $field['name'] ).'</a>';
    }
      echo "</div>";
    }
  }
  ...
  public function show_field_textarea( $field, $meta ) {
    $this->show_field_begin( $field, $meta );
      echo "<textarea class='at-textarea large-text".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}' id='{$field['id']}' ".( isset($field['style'])? "style='{$field['style']}' " : '' )." cols='60' rows='5'>{$meta}</textarea>";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_select( $field, $meta ) {
    ...
    if (isset($field['emptylabel']))
        echo '<option value="0">'.(isset($field['emptylabel'])? $field['emptylabel']: __('Select ...','breeze')).'</option>';
  ...
  public function show_field_wysiwyg( $field, $meta,$in_repeater = false ) {
    ...
    if ( $in_repeater )
      echo "<textarea class='at-wysiwyg theEditor large-text".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}' id='{$field['id']}' cols='60' rows='5'>{$meta}</textarea>";
    else{
   ...
   // $settings['quicktags'] = false;
    $settings['textarea_rows'] = isset($field['textarea_rows']) ? $field['textarea_rows'] : 7;
    $settings['teeny'] = true;
   ...
   */
  public function addRepeaterBlock($id,$args,$repeater=false){
    $new_field = array(
      'type'     => 'repeater',
      'id'       => $id,
  ...
  public function load_textdomain(){
    ...
    load_textdomain( 'breeze', dirname(__FILE__) . '/lang/' . get_locale() .'.mo' );
  }
} // End Class
