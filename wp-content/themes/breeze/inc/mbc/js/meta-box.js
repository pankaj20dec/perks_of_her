...
var _metabox_fields = {
  ...
  init: function(){
    ...
    $(document).on('click', ".at-re-toggle", function() {
      $(this).closest(".at-repater-block").find('.repeater-table').slideToggle();
    });
    // repeater sortable
  ...
  fancySelect: function(){
    ...
    if ($().select2){
	  console.log("sdsdd");
      $("select.at-select, select.at-posts-select, select.at-tax-select").each(function (){
        if(! $(this).hasClass('no-fancy'))
          $(this).select2().addClass("no-fancy");
      });
    }  
  },
